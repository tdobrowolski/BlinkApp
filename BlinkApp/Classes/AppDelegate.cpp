...
bool AppDelegate::applicationDidFinishLaunching() {
    ...
    if(!glview) {
        ...
        glview = GLViewImpl::createWithRect("BlinkApp", cocos2d::Rect(0, 0, designResolutionSize.width, designResolutionSize.height));
#else
        glview = GLViewImpl::create("BlinkApp");
#endif
    ...
    FileUtils::getInstance()->addSearchPath("res"); //dodaje sciezke "res"
    
    cocos2d::Size targetSize = glview->getFrameSize();
    
    std::vector<std::string> searchResolutionsOrder(1);
    
    if (targetSize.height < 481.0f) //ustalanie rozdzielczosci assetow w zaleznosci od rozdzielczosci ekranu
    {
